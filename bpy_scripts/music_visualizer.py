...
def main():
    ...
    cmd = ["ffmpeg", "-y", "-v", "error", "-framerate", f"{fps}/{step}", "-i", frames_dir + "/seq_%04d.png"]
    audio = cfg.get("audio_path")
    if audio:
        cmd += ["-ss", f"{cfg['start_seconds']:.3f}", "-t", f"{n / fps:.3f}", "-i", audio,
                "-map", "0:v", "-map", "1:a", "-c:a", "aac", "-b:a", "192k"]
    cmd += ["-r", str(fps), "-frames:v", str(n), "-pix_fmt", "yuv420p",
            "-c:v", "libx264", "-preset", "veryfast", cfg["out_path"]]
    mux = subprocess.run(cmd, capture_output=True, text=True)
    t_mux = time.time() - t0

    print("===== BPY MUSIC VISUALIZER RESULTS =====")
    print(f"scene: {cfg['scene']}  look: {cfg['look']}  intensity: {cfg['intensity']}")
    print(f"resolution: {cfg['width']}x{cfg['height']}")
    print(f"output_frames: {n} @ {fps}fps  frame_step: {step}  rendered_frames: {rendered}")
    print(f"song_window_s: {cfg['start_seconds']:.2f} -> {cfg['start_seconds'] + n / fps:.2f}")
    print(f"sections_in_window: {len(set(sig.section))}  beats_in_window: {len(set(b for b in sig.beat_index if b >= 0))}")
    print(f"setup_time_s: {t_setup:.2f}")
    print(f"render_time_s: {t_render:.2f}")
    print(f"render_time_per_rendered_frame_s: {t_render / rendered:.3f}")
    print(f"mux_time_s: {t_mux:.2f}  mux_ok: {mux.returncode == 0}")
    if mux.returncode != 0:
        print(mux.stderr)
    print(f"total_time_s: {time.time() - t_start:.2f}")
